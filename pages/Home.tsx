    ...
    <div className="space-y-12 pb-12">
      ...
      <div className="relative h-[400px] md:h-[500px] w-full overflow-hidden bg-gray-900 group">
        {carouselItems.length > 0 ? carouselItems.map((item, index) => (
          ...
          >
            <img src={item.imageUrl} alt={item.caption} className="w-full h-full object-cover opacity-70" />
            <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-black/90 to-transparent text-white">
              <div className="max-w-6xl mx-auto">
                <h2 className="text-3xl md:text-4xl font-bold mb-2">{item.caption}</h2>
              </div>
            </div>
          </div>
        )) : (
           <div className="flex items-center justify-center h-full text-white">No images available</div>
        )}
        
        {/* Controls - Only show if > 1 item */}
        {carouselItems.length > 1 && (
          <>
            <button onClick={prevSlide} className="absolute z-20 left-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 p-2 rounded-full backdrop-blur text-white transition">
              <ChevronLeft size={24} />
            </button>
            <button onClick={nextSlide} className="absolute z-20 right-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 p-2 rounded-full backdrop-blur text-white transition">
          ...
          {announcements.map((ann) => (
            ...
            >
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <h3 className="text-xl font-bold text-gray-800 mb-2 group-hover:text-blue-600 transition-colors">{ann.title}</h3>
                <p className="text-gray-600 mb-4 flex-grow line-clamp-3">{ann.content}</p>
                <span className="text-blue-600 text-sm font-medium mt-auto">Read Full Announcement &rarr;</span>
              </div>
            </Card>
          ))}
        </div>
        {announcements.length === 0 && <p className="text-gray-500 text-center">No announcements at this time.</p>}
      </div>
      ...
      >
        {selectedAnnouncement && (
            <div className="space-y-6">
                ...
                {/* Header */}
                <div>
                    <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">{selectedAnnouncement.title}</h2>
                    <div className="flex items-center text-gray-500">
                        <Calendar size={18} className="mr-2 text-blue-500" />
                        <span className="font-medium">{new Date(selectedAnnouncement.date).toLocaleDateString()}</span>
                    </div>
                </div>
                ...
                <div className="pt-6 border-t border-gray-100 flex justify-end">
                    <Button variant="outlined" onClick={() => setSelectedAnnouncement(null)}>Close</Button>
                </div>
            </div>
        )}
      </Modal>
