...
const Programs: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const [programsData, orgsData] = await Promise.all([
        supabaseDb.programs.getAll(),
        supabaseDb.organizations.getAll(),
      ]);
  ...
  const handleShare = async (program: Program, e: React.MouseEvent) => {
    ...
    const shareData = {
      title: program.title,
      text: `Check out ${program.title} by ${getOrgName(
        program.organizerId
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    setRegError("");
    try {
      const res = await supabaseDb.registrations.add({
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            Our ongoing initiatives and success stories.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          ...
          <Button
            ...
            className="no-print whitespace-nowrap w-full sm:w-auto"
          >
          ...
          filteredPrograms.map((program) => (
            <Card
              ...
              className={`flex flex-col md:flex-row ${
                program.imageUrl ? "h-auto md:h-64" : "h-auto"
              }`}
            ...
            >
              ...
              <div
                className={`p-6 flex flex-col justify-between ${
                  program.imageUrl ? "md:w-2/3" : "w-full"
                }`}
              >
                <div>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      {program.status && (
                        <span
                          className={`text-xs px-2 py-1 rounded uppercase font-bold tracking-wide ${getStatusColor(
                            program.status
                          )}`}
                        >
                          {program.status}
                      ...
                      {user && user.organizationId === program.organizerId && (
                        <span className="text-xs text-gray-500 font-medium no-print italic">
                          Managed by You
                        </span>
                      )}
                    </div>
                ...
                <div className="mt-4 flex gap-2 no-print">
                  <Button
                    variant="text"
                    className="pl-0"
                    onClick={(e) => handleShare(program, e)}
                  >
              ...
              regSuccess ? (
                <div className="flex flex-col items-center justify-center py-8 text-center animate-in fade-in">
                  ...
                  <p className="text-gray-600 mb-6">
                    ...
                    <strong>{selectedProgram.title}</strong>. <br />
                    <span className="text-sm text-orange-600 font-medium bg-orange-50 px-2 py-1 rounded mt-2 inline-block">
                ...
                >
                  ...
                  <div className="flex gap-3 pt-4">
                    ...
                    <Button type="submit" fullWidth isLoading={isSubmittingReg}>
              ...
              <>
                ...
                <div>
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
                    ...
                    {selectedProgram.status && (
                      <span
                        className={`text-xs px-3 py-1 rounded-full uppercase font-bold tracking-wide ${getStatusColor(
                          selectedProgram.status
                        )}`}
                      >
                        {selectedProgram.status}
                ...
                <div className="pt-6 border-t border-gray-100 flex flex-col sm:flex-row justify-end gap-2">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <Button
                      variant="text"
                      onClick={(e) => handleShare(selectedProgram, e)}
                    ...
                    {selectedProgram.status === "Upcoming" &&
                      selectedProgram.allowRegistration !== false && (
                        <Button
                          variant="contained"
                          color="primary"
